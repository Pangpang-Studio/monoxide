...
import { fontFileUrl, getPrebuiltMetadata, isStaticDeployMode } from './api'
...
async function startStaticApp(app: AppState) {
  ...
  try {
    ...
    let cmap = new Map<string, number>()
    for (let [k, v] of Object.entries(metadata.cmap)) {
      cmap.set(k, v)
    }
    app.renderedFont.value = {
...
export class AppState {
  ...
  public async reloadFontFace(): Promise<void> {
    ...
    const url = new URL(fontFileUrl(), window.location.href)
